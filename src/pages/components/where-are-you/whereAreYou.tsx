...
export default function WhereAreYou(
  { isLocationChosen, setTownId, setLocationChosen, townSearch, setTownSearch,  setUserGeoLocation }) {
  ...
  return (
    <div className='flex flex-col'>
      ...
      <div className='flex justify-between relative w-full'>
        <input required className='pl-2 border-2 h-10 text-xl' placeholder='Location' key={'inputBox'} onChange={(e) => onSearchChange(e.target.value)} type="text" name="" id="" value={townSearch} />
          ...
          <ul className='absolute left-0 w-full rounded-md border-1-black margin-0 padding-0'>
            ...
            {geoLocationSearchItems &&
              geoLocationSearchItems.map((item: GoogleMapsAutoSearchMappedData, index) => {
                return <li key={index} id={item.place_id} className='relative p-2 border-2 bg-slate-200 text-xl cursor-pointer border-2 rounded-sm border-slate-300' onClick={(e) => setLocationAndClearList({value: e.currentTarget.innerHTML, id: e.currentTarget.id})}>{item.place}</li>
              })
            }
          </ul>
        </div>
